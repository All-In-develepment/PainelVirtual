...
from painel.blueprints.billing.gateways.stripecom import \
...
class Invoice(ResourceMixin, db.Model):
    ...
    def parse_from_event(cls, payload):
        ...
        period_start_on = datetime.datetime.utcfromtimestamp(data['lines']['data'][0]['period']['start']).date()
        period_end_on = datetime.datetime.utcfromtimestamp(data['lines']['data'][0]['period']['end']).date()
    ...
    def parse_from_api(cls, payload):
        ...
        date = datetime.datetime.utcfromtimestamp( payload['lines']['data'][0]['period']['end'])

        invoice = {
            'plan': plan_info['product'],
            'description': plan_info['id'],
            'next_bill_on': date,
