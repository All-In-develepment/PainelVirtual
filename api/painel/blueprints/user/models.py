...
class User(UserMixin, ResourceMixin, db.Model):
    ...
    def initialize_password_reset(cls, identity):
        ...
        from painel.blueprints.user.tasks import (
    ...
    def serialize_token(self, expiration=3600):
        ...
        :return: JSON
        """
        private_key = current_app.config.get('SECRET_KEY')
        if not private_key:
            raise ValueError("SECRET_KEY não está definido.")

        serializer = URLSafeTimedSerializer(private_key, expiration)
        # A expiração deve ser passada no 'dumps'
        return serializer.dumps({'user_email': self.email}, salt='password-reset-salt')
